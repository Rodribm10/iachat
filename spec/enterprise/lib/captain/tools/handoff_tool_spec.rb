...
RSpec.describe Captain::Tools::HandoffTool, type: :model do
  ...
  # O caminho v2 do upstream so transfere conversa em `pending` — e o estado real
  # em que a IA esta atendendo.
  ...
  describe '#params' do
    # O upstream trocou o DSL: o schema agora e um bloco em `params do ... end`
    # em vez do hash `parameters`.
    it 'declara reason opcional e reason_category' do
      schema = described_class.params
      expect(schema).to be_a(RubyLLM::Tool::SchemaDefinition)
      expect(described_class::REASON_CATEGORIES).to include('customer_request')
    end
  end
